...
template <typename T>
void SinglyLinkedList<T>::PartitionRecursive(const T& pivot) {
    head_ = PartitionRecursiveHelper(head_, size_, pivot);
    ListNode* curr = head_;
    while (curr->next_ != nullptr) {
        curr = curr->next_;
    }
...
template <typename T>
typename SinglyLinkedList<T>::ListNode* SinglyLinkedList<T>::PartitionRecursiveHelper(
    ListNode* front,
    unsigned size,
    const T& pivot) {
    if (front == nullptr || front->next_ == nullptr) {
    ...
    front = PartitionRecursiveHelper(front, size/2, pivot);
    back = PartitionRecursiveHelper(back, size - size/2, pivot);
    return PartitionMergeHelper(front, back, pivot);
}

template <typename T>
typename SinglyLinkedList<T>::ListNode* SinglyLinkedList<T>::PartitionMergeHelper(
    ListNode* list1,
    ListNode* list2,
    const T& pivot) {
    ListNode* head = list1;
